...
from my_dgcnn import Backbone_Cls, MLP
...
class MyDGCNN_Cls(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.mlp = nn.Sequential(MLP([2 * args.emb_dims * 2, 512], args.norm),
                                 nn.Dropout(args.dropout),
                                 MLP([512, 256], args.norm),
                                 nn.Dropout(args.dropout))
        self.fc = nn.Linear(256, args.output_channels)

    def forward(self, x, pts, shape_norm):
        """
        :param x: (batch_size, input_channels, num_points)
        :return:
        """
        shape = self.backbone(x, pts)
        x = torch.concat([shape, torch.tile(shape_norm, (shape.shape[0], 1))], dim=1)
        out = self.fc(self.mlp(x))
...
class ToothDataset(Dataset):
        ...
        if train:
            ...
            for scene in os.listdir(data_path):
                if scene in self.test_scenes:
                    continue
                for fp in glob.glob(osp.join(data_path, scene, "*/11._Crown.stl")):
    ...
    def get_train_data(self, i):
        ...
        x0 = np.concatenate([pts, m.face_normals[fi]], axis=1).astype('f4')
        ...
        return x0.T, np.eye(3, dtype='f4').reshape(-1), x.T, mat.T.reshape(-1), tid, fp
...
class LitModel(pl.LightningModule):
    ...
    def __init__(self, epochs, batch_size, lr, num_pts, num_workers):
        ...
        self.mean_shape = torch.nn.parameter.Parameter(torch.zeros((1, 2 * args.emb_dims), dtype=torch.float32),
                                                       requires_grad=False)
        self.n_shape = 0
    ...
    def forward(self, x, pts):
        shape, out = self.net(x, pts, self.mean_shape.data)
        return shape, out

    def update_mean_shape(self, shape):
        self.mean_shape.data = self.mean_shape.data * 0.99 + shape.mean(0).detach() * 0.01
